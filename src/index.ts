...
export default class extends WorkerEntrypoint {
	async fetch(request: Request) {
		...
		// 		console.log('WWW', request.headers.get('Authorization'));
		...
		if(url.pathname.startsWith('/')) {
			...
			const back = await this.env.back.fetch(request)

			console.log('HASHED', auth);
			return(
				back.status === 200 ? await this.env.ASSETS.fetch(request): back
			);
		};
