...
const list = [
	["909", 100, 20, 20, material],
	["8383", 180, 30, 30, material],
	["3231", 180, 30, 30, material],
	["88800", 70, 70, 70, material],
	["230202", 70, 70, 70, material],
	["1298", 200, 5, 100, material],
	["123", 100, 5, 100, material],
	["5908", 150, 5, 90, material],
	["8899", 120, 3, 100, material],
	["777", 50, 3, 50, material],
	["8980", 30, 3, 30, material],
	["71234", 30, 3, 30, material],
	["1111", 30, 3, 30, material],
	["2313", 30, 3, 30, material],
	["1112", 60, 5, 90, material],
	["1897", 180, 5, 100, material],
	["9897", 75, 5, 80, material],
	["9884", 100, 5, 120, material],
	["8745", 130, 5, 100, material],
	["8877", 160, 5, 160, material],
	["34733", 130, 5, 50, material],
	["18988", 130, 5, 50, material],
	["38388", 130, 5, 50, material],
	["75784", 130, 5, 50, material],
	["90909", 100, 5, 90, material],
	["12345", 89, 5, 88, material],
	["98099", 120, 3, 100, material],
	["44444", 60, 5, 60, material],
	["98239", 40, 5, 50, material],
	["23984", 40, 5, 50, material],
	["999299", 40, 5, 50, material],
	["134144", 40, 5, 50, material],
	["121231", 50, 5, 50, material],
	["19023", 50, 5, 50, material],
	["9898", 50, 5, 50, material],
	["98888", 50, 5, 50, material],
	["11111", 60, 5, 60, material],
	["22222", 60, 5, 60, material],
	["33333", 60, 5, 60, material],
	["LJ-10298", 220, 10, 300, material],
	["LJ-298", 300, 10, 300, material],
];
	...
	backUp: [
		...
		[203, 53, 128, 229.525],
		{
			works: [
				...
				{
					layer2: [
						["9884", 100, 5, 120, 0.06, '<i class="nf nf-oct-sync"></i>'],
					],
				},
				{
					layer3: [
...
export const standard1 = [
	...
	["888", 30, 7, 50, 0.083],
	["88800", 130, 7, 80, 0.083],
	["88801", 100, 7, 110, 0.083],
];
...
function shiftAxios(work, layer) {
	...
	else if (work[0] <= layer[3] / 2 && work[1] <= layer[5]) return false;
...
function matchCanvasInLayer(matched, layer, arts, len) {
	if ((layer[0] === 0 && layer[2] === 0) || len < 0) return;
	const SPIN = 6;
	const FLIP = 2;
	let i = 0;
	let x = arts[len][1];
	...
	while (i++ < FLIP) {
		if (matchWorkOnLayer(x, y, layer)) {
			...
			if (i === 2 && arts[len].length < SPIN) arts[len].push(" ");
			else if (arts[len].length === SPIN) arts[len].pop();
...
// ╰───────────────────────────╯
export const canvas1 = [["22222", 260, 10, 260]];
...
// │ Tube variables. │
// ╰─────────────────╯
export const caseTube1 = [["22222", 260, 20, 20]];
...
// │ No canvas variables. │
// ╰──────────────────────╯
export const furniture0 = [["22126", 80, 80, 100]];
...
export const sculptore0 = [["22126", 80, 80, 100]];
...
function defineMaxWorks(items) {
	...
	items.map((art) => {
		...
		const check =
			(check1 > 0 && check1 <= PAD) || (check2 > 0 && check2 <= PAD);
...
export function provideNoCanvas(noCanvas) {
	...
	if (noCanvas.length == 0) return { noCanvas: false };
...
function tryComposeLayer(baseSize, list) {
	...
	const compLayer = list.map((size) => {
		...
		if (getter.length > 0) {
			if (checkComp(getter, size, baseSize)) return size;
...
export function provideSameSizeCanvas(work) {
	let crate;
	const result = sameSizeTrail(work);
...
function checkHugeTubes() {
	...
	const getter = this.filter((tube) => {
		if (tube[2] > DIAMETER) return tube;
		return;
	});
	getter.map((roll) => {
...
export function provideTubeCrate(tubes) {
	...
	if (!list) return { tube: false };
	if (tubes.length > 4) return crateTubesOpt(tubes);
	else crate = crateTubesOpt(tubes);
	tubes = null;
	return crate;
}
...
export function artWorksList(works) {
	...
	const artList = arts.map((work) => {
		return new ArtWork(work[0], work[1], work[2], work[3], material);
	});
...
export function findTubesTest(works) {
	...
	const tubes = noCanvas.filter((pece) => {
		if (pece[1] !== pece[2] && pece[2] === pece[3]) return pece;
		return;
	});
...
export function largestWorks(works) {
	...
	const largestCanvas = sorted.filter((work) => {
		if (work[1] > MAXHEIGHT || work[3] > MAXHEIGHT) return work;
		return;
	});
...
export function noCanvasOut(works) {
	...
	sameSize.filter((work) => {
		if (work[2] > MAXDEPTH) noCanvas.push(work);
	});
	noCanvas.map((art) => {
