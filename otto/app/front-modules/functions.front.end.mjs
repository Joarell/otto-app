...
export async function countWorks() {
	...
	result && result.length > 0
		? document
				...
				.setAttribute("content", result.length)
		: false;
...
export async function crate(fetched = false) {
	...
	if (fetched || (confirm("Ready to crate all works?") && cratesAsCm)) {
		...
		grant === "FULL" || grant === "PLOTTER" || !grant
			? (document.getElementById("crate-layers").disabled = false)
			: 0;
...
async function addPanelInfoData() {
	...
	sideMenu.children.length === 2
		? sideMenu.removeChild(sideMenu.lastElementChild)
		: false;
	return sideMenu.children.length === 1
...
export async function cleanInputs(fetched = false) {
	...
	packs.getAttribute("name") !== "update-materials"
		? packs.setAttribute("name", "update-materials")
		: 0;
	countWorks();
	displayCub();
	displayAirCub();
...
async function checkMetric() {
	...
	if (list.length === 0)
		return alert(
			"Oops! Sounds like you do not added any work yet. Please, try again!",
		);
