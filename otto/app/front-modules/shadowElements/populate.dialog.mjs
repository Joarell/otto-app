...
export async function populateCrates(frame) {
	...
	for (option in crates) {
		if (Object.hasOwn(crates[option], "crates")) {
...
export async function alterCrateSizes(list) {
	...
	for (item of list.childNodes) {
		item.childNodes[0].childNodes[0].checked
			? selectedCrates.push(crate)
			: false;
		crate++;
	}
	return selectedCrates.length === 0
...
async function updateCrateSizes(list) {
	...
	Object.entries(crates).map((data) => {
		if (Object.hasOwn(data[1], "crates")) {
			const { crates } = data[1];
...
function addNewSize(crate) {
	...
	if (sizes.includes("")) return crate;
...
async function updateCratesData(data, newSizes) {
	const WORKER = new Worker(
		new URL("../worker.storage.save.mjs", import.meta.url),
		{ type: "module" },
	);
	let airCubTotal = 0;
	let PAX = 0;
