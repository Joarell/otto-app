...
export class DialogPadding extends HTMLElement {
	...
	async #populatePaddingCrate() {
		...
		shadowRoot
			.getElementById("padding-close")
			.addEventListener("click", this.close);
		shadowRoot.getElementById("padding-apply").addEventListener("click", () => {
	...
	async #populateStatus() {
		...
		return table
			? await statusTable(table, true, result)
	...
	async #populatePane1(update = false) {
		...
		if (update)
			while (cratePane.firstChild) cratePane.removeChild(cratePane.firstChild);
	...
	async #populatePane2(update = false) {
		...
		if (update)
			while (openPane.firstChild) openPane.removeChild(openPane.firstChild);
	...
	async attributeChangedCallback(attrName, oldVal, newVal) {
		const shadowRoot = this.#shadowRoot.get(this);
		const status =
			oldVal !== newVal && attrName === "content" && newVal !== "FETCHED";
		...
		switch (attrName) {
			case "padding":
				return oldVal
					? await populateCrates(shadowRoot.querySelector("#crate-list"))
					: 0;
			case "content":
	...
	async apply() {
		...
		storage.setItem(
			"SetCrates",
			JSON.stringify(structuredClone([X.value, Z.value, Y.value])),
		);
	...
	close() {
		...
		closeDialog.getElementsByTagName("panel-info").length > 0
			? document.querySelector(".side-menu").lastElementChild.remove()
			: false;
	}
}
