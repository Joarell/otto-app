export default class CrateTrimmer {
	...
	#quickSort(list) {
		...
		list.map((crate) =>
			crate.at(-1) <= pivot.at(-1) ? left.push(crate) : right.push(crate),
		);
	...
	#reuseMaterialRecursion(data, sizes, i = 0) {
		...
		if (!data[0][0] || i === 2 || (checker.a && checker.b)) return data[0];
		...
		if (material[0] >= sizes[0] && material[1] >= sizes[1]) {
			...
			material[0] =
				material[0] === sizes[0] ? 0 : +(material[0] - sizes[0]).toFixed(3);
			if (!material[0] && material[1] === sizes[1])
				material[1] = +(material[1] - sizes[1]).toFixed(3);
			if (!material[0] && material[1]) {
				if (material[1] > sizes[1]) {
					material[0] = x;
					material[1] = +(material[1] - sizes[1]).toFixed(3);
				} else material[0] = +(material[0] - sizes[0]).toFixed(3);
			}
			const diff = material[0] !== x || material[1] !== y;
			if (diff && !checker.a) checker.a = true;
			else checker.b = true;
			if (!checker.b && material[0] >= sizes[0] && material[1] >= sizes[1]) {
				...
				if (material[0] !== x && !checker.a) checker.a = true;
				else checker.b = true;
			}
		}
	...
	#cutterManager(provided, bank, crate) {
		...
		const takingMaterial = (sheet) => {
			sheet = this.#reuseMaterialRecursion(
				[sheet, faceAB],
				[crate[0], crate[2]],
			);
			sheet = this.#reuseMaterialRecursion(
			...
			if (sheet[0] <= 10 || sheet[1] <= 10) sheet = 0;
			return sheet;
		};
		let checker = false;
		let stored = true;
	...
	#largeCrateStructure(crate, pine) {
		...
		const allPine = +((baseEnforce + highEnforce) / pine[1]).toFixed(3);
		const bottom = {
	...
	#foamDivisorLayers(size, foam) {
		...
		foam.map((pad) => {
			...
			if (pad[0] >= size[0]) x = +(pad[0] - size[0]).toFixed(3);
			if (pad[0] === size[0]) y = +(pad[1] - size[2]).toFixed(3);
			else y = pad[1];

			if (!x && pad[1] >= size[0]) x = +(pad[1] - size[0]).toFixed(3);
			if (!y && pad[1] === size[0]) y = +(pad[0] - size[1]).toFixed(3);
			else y = pad[0];
			trimmed.push([x, y]);
			return pad;
		});
	...
	async #fillEmptySpaceOnLayers(crate) {
		let foam2 =
			this.#materialBank.get("2") ||
			this.#crateMaterials.find((item) => {
				if (item[5] === "Foam Sheet" && item[2] <= 2.5) return item;
				return item;
			});
		let foam5 =
			this.#materialBank.get("5") ||
			this.#crateMaterials.find((item) => {
				if (item[5] === "Foam Sheet" && item[2] > 2.5) return item;
				return item;
			});
		...
		const results = calcLayer
			.flatMap((info) => {
				const { highestZ, total } = info;
				return foam5 && highestZ[1] >= +foam5[2]
					? [foam5, total]
					: [foam2, total];
			})
	...
	async #trimmingPaddingLayersMaterials(foam, crate, index) {
		...
		const areaBank = onBank ? onBank.map((val) => val[0] * val[1]) : 0;
		const foamArea = +(+foam[1] * +foam[3] - areaBank).toFixed(3);
		const count = Math.ceil(layerArea / foamArea);
		const { provision, extraX, extraY } = this.#provideMaterial(
			count,
			[+foam[1], +foam[3]],
			innerSize,
		);
		...
		residual =
			!residual && count
				? this.#foamDivisorLayers(innerSize, provision)
				: residual;
	...
	#trimmingMainCrateMaterial(item, crate, index) {
		...
		const areaBank = onBank
			? Number.parseFloat(
				onBank[1].reduce((val, sum) => sum + +val[0] * +val[1], 0),
			)
			: 0;
		const materialArea = areaBank
			? +item[1] * +item[3] - +areaBank
			: +item[1] * +item[3];
