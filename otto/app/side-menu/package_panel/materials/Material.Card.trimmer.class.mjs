export default class PackingTrimmer {
	...
	#defineAvailableCoordinates(work, material, counter) {
		...
		if (pack) {
			const coordinates =
				+pack[2] >= +pack[4]
					? [[+pack[2] * counter, +pack[4]]]
					: [[+pack[4] * counter, +pack[2]]];
	...
	#feedMaterialOnBanck(retang, found, optimized, feat, count) {
		if ((!found && !optimized[1]) || !feat) {
			const positions = this.#defineAvailableCoordinates(
				retang,
				optimized,
				count,
			);
	 ...
	 */
		...
		let area =
			available && Array.isArray(available[0][1])
				? available.find((opt) => opt[1][0] && opt[1][1])
				: false;
	...
	async #cutMaterial(packedInfo, list, i = 1) {
		try {
			if (!list[i]) return this.#plannedWorks;
			const art = list[i];
			...
			const materials = pack.filter((kind, j) => {
				if (kind[2] === reuse[j][0] && reuse[j][1]) return kind;
			}, 0);
	...
	get cardBoardCutter() {
		this.#sorted.reverse().map(async (check, i) => {
			if (i) return await this.#cutMaterial(check, this.#plannedWorks);
			const firstCheck = this.#plannedWorks.find(
				(work) => work.code === check[0],
			);
