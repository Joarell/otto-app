...
export class PackageInfoUp extends HTMLElement {
	 ...
	 */
	#inputListener() {
		globalThis.document.querySelector(".package-crates").addEventListener(
			"input",
			async (element) => {
				const { className } = element.target;
				const value = element.target.getAttribute("content");
				const customElement =
					element.target.shadowRoot.querySelector(".select-materials");

				if (value !== 0 && className === "packing-materials") return;
				if (customElement)
					customElement.addEventListener(
						"input",
						this.#localStoreSelectedMaterials,
						true,
					);
			},
			true,
	...
	async #toggleReportDownPanel(work) {
		...
		const onCrate = new Map(
			Object.entries(JSON.parse(globalThis.sessionStorage.getItem("onCrate"))),
		);
	...
	async #checkMaterialsAvailable() {
		this.#inputListener();
		if (await new AvailableMaterials().allMaterials) return true;
	...
	async #populateMaterialsUpPanel() {
		if (!(await this.#checkMaterialsAvailable())) return true;
		this.#hiddenContent();
	...
	#removeField() {
		...
		className && className === "material-sizes"
			? newMaterials.removeChild(newMaterials.lastElementChild)
			: removeMaterial.setAttribute("name", "remove-material");
	...
	async attributeChangedCallback(attName, oldVal, newVal) {
		...
		switch (newVal) {
			...
			case "packages":
				this.#inputListener();
				return className !== "update-materials" &&
					shadowClass !== "select-materials"
					? this.#cleanPanel(id, shadowClass)
					: 0;
			case "select-materials":
				this.#inputListener();
				return className !== "update-materials" &&
			...
			case "materials":
				...
				return className !== "update-materials" &&
					...
					: 0;
			case "pack-opts":
				return shadowClass !== "upPane"
					? await this.#populatePackedWorksInCrates()
					: 0;
			case "works-packed":
				return shadowClass !== "upPane"
					? await this.#populatePackedWorksInCrates()
