...
export default class UsedMaterialsTable {
	...
	#imutableTableSetup() {
		...
		worksReport.finalReport.map((item) => {
			...
			const { totalCost, residual, counter, type, area } = item[1];
	...
	async #materialsOnTable(artWork, table) {
		if (!Array.isArray(artWork)) return table;
		const { demand, percent, reuse, residual, types, cost } = artWork[1];
	...
	#correctCutMaterials(info, layers) {
		...
		const div = this.#materials.find(
			(item) => item[2] <= 2.5 && item[5] === "Foam Sheet",
		);
		const feet = 8;
	...
	#crateMaterialCut(crate, table) {
		...
		return crate[1] === "largestCrate"
			? this.#largestCrateTable(crate, table)
			: table;
	}
	...
	#fillLayerTable(table, data, gap, material) {
		...
		const sizes = data.total
			? `<td> ${gap[0]} x ${gap[1]} - ${metric}</td>`
			: `<td>N/A</td>`;
		const area = data.total
			? `<td> ${((gap[0] * gap[1]) / 100).toFixed(2)} m²</td>`
			: `<td>0 m²</td>`;
	...
	#layerTable(node, data, crateNumb) {
		...
		if (avoid.includes(data[1])) return node;
	...
	async #setsTableElements(node) {
		if (!this.#materialReport) return false;
	...
	async #populateTableMaterialsUesed() {
		...
		if (!info || !(await this.#setsTableElements(table))) return false;
