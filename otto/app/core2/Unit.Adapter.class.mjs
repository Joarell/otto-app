...
export default class UnitAdapter {
	...
	async #definePath() {
		...
		const path =
			checker !== "pass" ? "error" : this.#unit === "cm" ? "cm" : "in";
	...
	#convertToIN(crates) {
		...
		if (crates.sameSizeCrate?.hasOwnProperty("backUp")) {
			crates.airCubTotalBackUp = +(crates.airCubTotalBackUp * CUBCONST).toFixed(
				3,
			);
	...
	async #cmPath() {
		const RESULT = await Promise.resolve(new Arranger(this.#list).start)
	...
	async #inPath() {
		const RESULT = await Promise.resolve(this.#convertToCM())
			.then((list) => new Arranger(list).start)
			.then((procList) => new Crater(procList, this.#materials).makeCrate)
...
function layerInterface(layer) {
	...
	for (key in layer) {
		Array.isArray(layer[key][0][0][0])
			? layer[key][0].map((work) =>
					work.splice(0, 1, swapUnitReversion(work[0])),
				)
