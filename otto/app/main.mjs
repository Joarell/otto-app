import { clearAll, crate, setUnit } from "./front-modules/checkout.mjs";
...
globalThis.onkeydown = (push) => {
	...
	const task2 =
		push.ctrlKey === true && push.altKey === true && push.key === "c";
...
globalThis.document.getElementById("main-app").addEventListener(
	"click",
	(element) => {
		...
		switch (!id ? (id = className) : id) {
			...
			case "settings-content":
				className !== "update-materials" && className !== "new-material"
					? up.setAttribute("content", "settings-content")
					: 0;
				break;
			case "packages":
				className !== "update-materials" && className !== "select-materials"
					? up.setAttribute("content", "packages")
					: 0;
				break;
			case "select-materials":
				className !== "update-materials" && className !== "select-materials"
					? up.setAttribute("content", "select-materials")
					: 0;
				break;
			case "materials":
				className !== "update-materials" && className !== "select-materials"
					? up.setAttribute("content", "confirm-save")
					: 0;
				break;
			case "report":
...
globalThis.document
	.getElementById("estimate_getter")
