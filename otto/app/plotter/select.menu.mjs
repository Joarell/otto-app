...
async function unitAdapterSetCrates(unit, solved) {
	if (unit === "in") {
		...
		Object.entries(crates).map((data) => {
			...
			for (i in data) {
				if (Object.hasOwn(data[i], "crates")) {
...
export async function layersNumber(list) {
	...
	const data = list ?? JSON.parse(localStorage.getItem("doneList"));
	let number;
	let key;

	for (key in data) {
		if (Object.hasOwn(data[key], "crates") && selected > 0) {
