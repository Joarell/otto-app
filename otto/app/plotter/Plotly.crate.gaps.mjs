export default class FillGaps {
	...
	#defineFillSize({ x, y, z }, locate) {
		...
		const offY = !locate.y ? y + this.#data.offset[2] - this.#data.pad[2]:
		...
		const divisor = {
			...
			offsetZ: locate.y ? locate.y + this.#data.offset[2] - this.#data.pad[2]:
	...
	#sizesToFill() {
		...
		const gaps =		vacuum.reverse().map((data, i) =>  {
			...
			this.#defineGapSize(data, lastValues): 0;
		}, 0);
		const sizes =	vacuum.map((gap, i) => {
			...
			const fill = {
				...
				size: {
					...
					z: this.#data.thickness > this.#data.div[2] ?
						this.#data.pad[2]: this.#data.div[2],
				},
			};
			lastX = gap[0];
			lastY = gap[1];
	...
	#defineGaps() {
		...
		const gaps =		allSizes.map(data => this.#defineFillSize(data.size, data.location));
		return(gaps);
	};
