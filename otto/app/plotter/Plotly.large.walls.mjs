...
export default class SetLargeCrateWalls {
	#data;
	#pine;
	#ply;
	#crate;
	...
	#crateWalls() {
		...
		const height = +this.#feet[3] + 4 * this.#ply[2] + +this.#pine[2];
	...
	#offsetWalls() {
		const structure = +this.#feet[3] + 3 * this.#ply[2];
		const allOffSet = {
			faceBack: {
				type: "backFace",
				x: +this.#pine[2],
				y: 2 * this.#ply[2] + this.#pine[2] + structure,
				z: 2 * +this.#pine[2],
				width: this.#crate[0] - 2 * this.#pine[2],
				depth: +this.#pine[2],
				height: this.#crate[2] - (3 * this.#ply[2] + structure + 2 * +this.#pine[2]),
				offsetX: +this.#pine[2],
				offsetY: +this.#pine[2],
				offsetZ: +this.#feet[3] + structure + this.#ply[2],
			},
			faceFront: {
				type: "frontFace",
				x: +this.#pine[2],
			...
			bottom: {
				type: "bottom",
				x: this.#pine[2],
				y: structure + 2 * this.#ply[2] + +this.#pine[2],
				z: this.#pine[2],
				width: this.#crate[0] - 2 * +this.#pine[2],
				depth: this.#crate[1] - 2 * +this.#pine[2],
				height: +this.#pine[2],
	...
	get setHugeWalls() {
		return this.#defineHugeCrateWalls();
	}
}
