...
function getAirport(crates) {
	...
	pax = crates.filter((g_crates) => {
		if (
			g_crates[1] <= pax_lim[0] &&
			g_crates[2] <= pax_lim[1] &&
	...
	cargo = crates.filter((v_crates) => {
		if (
			v_crates[1] >= pax_lim[0] ||
			v_crates[2] >= pax_lim[1] ||
...
export function boss(the_list) {
	...
	if (largest.length !== 0) {
		...
		else crates = man.largest(largest, crates);
	} else next_work.noCanvasOut(proc_list, layer, largest);
