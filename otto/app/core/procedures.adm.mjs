...
function defineCrateSizes(inner_size, layers, weight) {
	...
	x = 23 + inner_size[0];
	if (weight > 10) z = 23 * layers + weight;
	if (layers > 1) z = 23 * layers;
	else z = 23 + inner_size[1];
	if (inner_size.length > 2) y = 28 + inner_size[2];
	else y = 28 + inner_size[1];
...
export function finishedDimensions(crates_done) {
	...
	while (crates_done.length > 0) {
		if (crates_done[0] > aux) aux = crates_done[0];
...
export function lastStep(layer_size, list, len, storage) {
	if (list.length <= 0) return storage;
